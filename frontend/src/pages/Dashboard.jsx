...
export default function Dashboard() {
  ...
  const [cronogramaAbierto, setCronogramaAbierto] = useState(null);
  const [rol, setRol] = useState("cliente");

  useEffect(() => {
    const u = localStorage.getItem("usuario");
    const token = localStorage.getItem("token");
    const r = localStorage.getItem("rol") || "cliente";
    setRol(r);
  ...
  const handleExito = (solicitud) => {
    setModalOpen(false); setExito(solicitud);
    setSolicitudes(prev => [solicitud, ...prev]);
  };

  const handleLogout = () => {
    localStorage.removeItem("token"); localStorage.removeItem("usuario"); localStorage.removeItem("rol");
    window.location.href = "/";
  };

  const generarCronograma = (monto, plazo, tea, cuota) => {
    const tem = Math.pow(1 + tea / 100, 1 / 12) - 1;
    const filas = []; let saldo = Number(monto);
    for (let n = 1; n <= plazo; n++) {
      const interes = saldo * tem;
      const capital = Number(cuota) - interes;
      saldo = Math.max(0, saldo - capital);
      const fecha = new Date(new Date().getFullYear(), new Date().getMonth() + n, new Date().getDate());
      filas.push({ n, fecha: fecha.toLocaleDateString("es-PE"), cuota: Number(cuota).toFixed(2), capital: capital.toFixed(2), interes: interes.toFixed(2), saldo: saldo.toFixed(2) });
    }
    return filas;
  };

  const nombreMostrar = usuario?.nombre && usuario.nombre !== "Cliente"
    ? usuario.nombre
    : usuario?.email?.split("@")[0] || "Cliente";

  const horaActual = new Date().getHours();
  const saludo = horaActual < 12 ? "Buenos dias" : horaActual < 18 ? "Buenas tardes" : "Buenas noches";
  ...
  // Pantalla de bienvenida para ASESOR
  if (rol === "asesor" || rol === "admin" || rol === "comite") {
    return (
      <div style={styles.page}>
        <nav style={styles.nav}>
          <div style={styles.navLeft}>
            ...
            <div>
              ...
              <div style={styles.logoSub}>Portal Interno</div>
            </div>
          </div>
          <div style={styles.navRight}>
            <div style={{...styles.rolBadge, background: rol==="admin"?"#fef2f2":rol==="comite"?"#f5f3ff":"#e0f2fe", color: rol==="admin"?"#dc2626":rol==="comite"?"#7c3aed":"#0369a1"}}>
              {rol==="admin"?"👑 Administrador":rol==="comite"?"⚖️ Comite":"🧑‍💼 Asesor"}
            </div>
        ...
        <div style={{...styles.hero, background:"linear-gradient(135deg,#1e3a5f,#0369a1)"}}>
          <h1 style={styles.heroTitle}>{saludo}, {nombreMostrar} 👋</h1>
          <p style={styles.heroSub}>Panel de gestion interna — {rol==="admin"?"Acceso completo":rol==="comite"?"Resolucion de comite":"Atencion al cliente"}</p>
        </div>

        <div style={styles.content}>
          <h2 style={{fontSize:18,fontWeight:700,color:"#0f172a",marginBottom:20}}>Acceso rapido</h2>
          <div style={styles.asesorGrid}>
            {[
              {icon:"📋",title:"Bandeja de Solicitudes",desc:"Gestiona todas las solicitudes de credito",color:"#0ea5e9",action:()=>window.location.href="/core"},
              {icon:"⚠️",title:"Mora y Recuperacion",desc:"Revisa y gestiona la cartera morosa",color:"#d97706",action:()=>window.location.href="/core#mora"},
              {icon:"🏦",title:"Captaciones / Ahorros",desc:"Administra las cuentas de ahorro",color:"#059669",action:()=>window.location.href="/core#ahorros"},
              {icon:"👥",title:"Propuesta y Comite",desc:"Solicitudes pendientes de resolucion",color:"#7c3aed",action:()=>window.location.href="/core#comite"},
              {icon:"💰",title:"Aprobacion y Desembolso",desc:"Creditos aprobados listos para desembolsar",color:"#1e3a5f",action:()=>window.location.href="/core#desembolso"},
              {icon:"📊",title:"Panel Core Completo",desc:"Accede al panel completo del sistema",color:"#0f172a",action:()=>window.location.href="/core"},
            ].map((item,i)=>(
              <div key={i} style={styles.asesorCard} onClick={item.action}>
                <div style={{...styles.asesorIcon,background:item.color+"15",color:item.color}}>{item.icon}</div>
                <div style={styles.asesorTitle}>{item.title}</div>
                <div style={styles.asesorDesc}>{item.desc}</div>
                <div style={{...styles.asesorArrow,color:item.color}}>Acceder →</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={(e)=>e.target===e.currentTarget&&setDepositoModal(false)}>
      <div style={{background:"#fff",borderRadius:16,padding:28,width:360,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        ...
        <div style={{marginBottom:16}}>
          <label style={{display:"block",fontSize:13,fontWeight:500,color:"#374151",marginBottom:6}}>Monto (S/)</label>
    ...
    <div style={styles.page}>
      ...
      <nav style={styles.nav}>
        ...
        <div style={styles.navLinks}>
          {["Inicio","Mis Cuentas","Prestamos","Transferencias","Pagos"].map(l=>(
            <a key={l} style={styles.navLink}>{l}</a>
          ))}
        </div>
        <div style={styles.navRight}>
          <div style={{...styles.rolBadge,background:"#ecfdf5",color:"#059669"}}>👤 Cliente</div>
          <div style={styles.userBadge}>{(usuario.nombre||usuario.email)?.[0]?.toUpperCase()}</div>
          <button style={styles.btnLogout} onClick={handleLogout}>Salir</button>
        </div>
      </nav>
      ...
      <div style={styles.hero}>
        <h1 style={styles.heroTitle}>{saludo}, {nombreMostrar} 👋</h1>
        <p style={styles.heroSub}>Aqui tienes un resumen de tu actividad financiera</p>
      </div>
      ...
      <div style={styles.content}>
        ...
        <div style={styles.cards}>
          {[
            ...
            {label:"Creditos Activos",value:solicitudes.filter(s=>s.estado!=="rechazado").length.toString(),sub:"Solicitudes activas",icon:"📋",color:"#8b5cf6"},
            {label:"Proxima Cuota",value:`S/ ${solicitudes.find(s=>s.estado==="desembolsado")?.cuota_mensual||solicitudes[0]?.cuota_mensual||"0.00"}`,sub:solicitudes.length?"Ver detalle abajo":"Sin cuotas",icon:"📅",color:"#059669"},
          ].map((c,i)=>(
        ...
        <div style={styles.chartsRow}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:8}}>
              {[
                ...
                <div key={i} style={{display:"flex",alignItems:"center",gap:10}}>
                  ...
                  <div style={{width:`${Math.min(item.count*40,160)}px`,height:18,background:item.color,borderRadius:4,opacity:0.7,minWidth:4}}/>
          ...
          <div style={styles.chartCard}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10,marginTop:8}}>
              {[
                {label:"Depositos",monto:movimientos.filter(m=>m.tipo==="deposito").reduce((a,m)=>a+Number(m.monto),0),color:"#059669"},
                {label:"Creditos",monto:movimientos.filter(m=>m.tipo==="credito").reduce((a,m)=>a+Number(m.monto),0),color:"#1e3a5f"},
                ...
                <div key={i} style={{display:"flex",alignItems:"center",gap:10}}>
                  <div style={{width:12,height:12,borderRadius:"50%",background:item.color,flexShrink:0}}/>
                  <div style={{flex:1,fontSize:13,color:"#374151"}}>{item.label}</div>
                  <div style={{width:`${Math.min(item.monto/40,160)}px`,height:18,background:item.color,borderRadius:4,opacity:0.7,minWidth:4}}/>
                ...
                <div key={i} style={{borderBottom:"1px solid #f1f5f9"}}>
                  <div style={styles.solicitudRow}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <span style={{padding:"5px 12px",borderRadius:20,fontSize:12,fontWeight:600,background:estadoBg[s.estado]||"#f1f5f9",color:estadoColor[s.estado]||"#64748b"}}>{s.estado}</span>
                      {s.estado==="desembolsado"&&(
                        <button style={{background:abierto?"#1e3a5f":"#e0f2fe",color:abierto?"#fff":"#1e3a5f",border:"none",borderRadius:8,padding:"6px 14px",cursor:"pointer",fontSize:12,fontWeight:600}} onClick={()=>setCronogramaAbierto(abierto?null:s.id)}>
                  ...
                  {abierto&&(
                    <div style={{padding:"16px 0 20px",overflowX:"auto"}}>
                      <div style={{fontSize:13,fontWeight:700,color:"#1e3a5f",marginBottom:10}}>Cronograma de Pagos — {s.plazo_meses} cuotas de S/ {Number(s.cuota_mensual).toFixed(2)}</div>
                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                        <thead>
                          <tr style={{background:"#f8fafc"}}>
                            {["N","Fecha","Cuota","Capital","Interes","Saldo"].map(h=>(
                        ...
                        <tbody>
                          {cronograma.map((f,j)=>(
                            <tr key={j} style={{background:j%2===0?"#fff":"#f8fafc"}}>
                              <td style={{padding:"7px 12px",color:"#64748b"}}>{f.n}</td>
                              <td style={{padding:"7px 12px",color:"#374151"}}>{f.fecha}</td>
                              <td style={{padding:"7px 12px",fontWeight:700,color:"#0f172a"}}>S/ {f.cuota}</td>
                              <td style={{padding:"7px 12px",color:"#1e3a5f"}}>S/ {f.capital}</td>
                              <td style={{padding:"7px 12px",color:"#dc2626"}}>S/ {f.interes}</td>
                              <td style={{padding:"7px 12px",color:"#059669"}}>S/ {f.saldo}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
...
const styles = {
  ...
  rolBadge:{padding:"4px 12px",borderRadius:20,fontSize:12,fontWeight:600},
  ...
  chartCard:{background:"#fff",borderRadius:16,padding:24,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"},
  ...
  asesorGrid:{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20},
  asesorCard:{background:"#fff",borderRadius:16,padding:24,boxShadow:"0 2px 8px rgba(0,0,0,0.06)",cursor:"pointer",transition:"all 0.2s",border:"1.5px solid #e2e8f0"},
  asesorIcon:{width:48,height:48,borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,marginBottom:14},
  asesorTitle:{fontSize:15,fontWeight:700,color:"#0f172a",marginBottom:6},
  asesorDesc:{fontSize:13,color:"#64748b",marginBottom:12,lineHeight:1.5},
  asesorArrow:{fontSize:13,fontWeight:600},
};
